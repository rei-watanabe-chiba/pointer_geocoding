...
* **main_dock_dialogs.py**: `main_dock.py`系の独立ダイアログ/ウィジェットクラス群。`ModelessSectionDialog`（設定/出力ダイアログ用の汎用モードレスラッパー。事前構築済みコンテンツウィジェットを表示するだけで業務ロジックを持たず、`on_show`/`on_close`コールバックで`MainDockWidget`へ表示状態を通知する）、`ImageDialog`（画像管理フォーム＋埋め込み基準点プレビュー`QgsMapCanvas`を1画面に統合したモードレスダイアログ。旧`PreviewDialog`を統合・置換、T-0024）、`TwoDigitSpinBox`（00-99表示スピンボックス）、`GridInputDialog`（基準点グリッド入力モーダル）、`FeatureCreateDialog`（T-0027: 新規遺構名作成モーダル。テキスト入力欄のみ、OK/キャンセル）、`PointNameEntryDialog`（T-0040: 新規モードで「自動連番/解除」トグルが解除側の時のキャンバスクリックに対し、クリック地点付近にポップアップして点名/枝番を専用入力させるモーダル。属性がSPの場合は`edit_point_name_sp`と同じ正規表現`^[A-Za-z0-9_-]+$`のQLineEdit、それ以外は整数QSpinBoxに切り替わり、初期値には同一グループ内の直近作成点名を提示する（T-0041）。OKで`core_logic.check_point_duplicate`による重複チェックを行い、重複時はOK/キャンセルボタンの上に赤字で表示して閉じない）を提供する。既存打刻点自体の点名/枝番/属性変更は、T-0038で本ダイアログ方式（旧`PointRenameDialog`、廃止）からフォーカスアウト時のリアルタイムコミット方式（`tab2_digitizing_mixin.py`側、2.3節参照）へ置き換わっている。`main_dock_constants.py`と`style_helper.py`・`core_logic.py`・`map_tool.py`にのみ依存する。
...
* **tab2_digitizing_mixin.py**: `Tab2DigitizingMixin`。Tab2 (点情報/属性/フォーカスモード/図面選択リストの4常時展開パネル、T-0027) のUI構築(`_create_tab2_ui`)と、パネル先頭の新規/編集モード切替トグル(`self.tab2_current_mode`、T-0036)・フォーカスモード制御・カテゴリ選択・新規モードでの打刻(`_on_canvas_clicked`/`_handle_release_mode_click`)・編集モードでの既存点削除(`_on_delete_selected_point`)/選択解除(`_on_blank_click_in_edit_mode`)・既存点の点名/枝番/属性系のリアルタイムコミット(`_commit_point_identity_if_editing`/`_commit_attribute_fields_if_editing`、T-0038)までの一連のイベントハンドラを提供する`MainDockWidget`用Mixin。CSV出力UI(`_create_tab4_ui`。旧`_create_output_ui`、T-0034でリネーム)とそのハンドラ(`_browse_csv_path`/`_on_export_csv_clicked`)も同モジュールが提供するが、T-0024よりこの出力UIは`tab2_container`（メインエリア）ではなく独立した`self.output_dialog`に格納される。
...
**T-0027 (アコーディオン全廃止・4パネル常時展開化)**: 旧`QgsCollapsibleGroupBox`3枠（フォーカスモード/入力カテゴリ/個別入力）+独立のステータスパネル構成を廃止し、常時展開の`QGroupBox`4枠に再構成した:
  * **① 点情報パネル (`group_point_info`)**: グリッド/遺構名・属性・番号枝番(`edit_point_name`/`edit_point_name_sp`/`edit_branch_no`)・XY座標(既存点選択時のみ表示、`core_logic.to_survey_coords`で測量座標表示)を表示。点名/枝番入力欄はT-0032以降、既存点選択中も`setEnabled(False)`にはならず常時編集可能である（編集中の値の扱いはT-0038、後述の「T-0036〜T-0042」段落を参照）。ボタンエリアは、パネル最上部に置かれた新規/編集モードトグル(`tab2_mode_container`、`self.tab2_current_mode`、T-0036)に連動して`widget_new_mode_actions`（新規モード時: `tab2_autonum_container`=[自動連番][解除]の疑似トグル、`self.tab2_autonum_mode`）と`widget_edit_mode_actions`（編集モード時: 「削除」ボタン`btn_delete_point`のみ、確認ダイアログなしで即削除）のいずれか一方のみが表示される。旧「点名変更」ボタン(`row_existing_actions`、`main_dock_dialogs.PointRenameDialog`を開く方式)はT-0038で廃止された。
  * **② 属性パネル (`group_attribute_panel`)**: 対象図面(`combo_drawing_name`)・出土形態(`combo_excavation_type`)・遺構名セレクタ(`combo_feature_name`)+「作成」ボタン(`btn_create_feature`、`main_dock_dialogs.FeatureCreateDialog`を開く)・カラーピッカー(`btn_color_picker`、遺構名が具体的に選択されている時のみ表示、`QColorDialog`のOKで即座に全該当フィーチャへ反映)・属性記号(`combo_attribute`)。旧`edit_new_feature`(常時表示の新規遺構名入力欄)・`btn_apply_color`(グループ一括適用ボタン)は廃止。既存点選択中(編集モード)のこれらウィジェットの変更もT-0038でリアルタイムコミット対象になったため、旧「属性変更」確定ボタンは持たない。
...
**T-0036〜T-0042 (新規/編集モードによるキャンバス操作・入力コミット方式の分離)**: ①のモードトグル(`tab2_current_mode`)は、ボタンエリア表示だけでなくキャンバス上のクリック/ホバー挙動と、既存点の項目変更のコミット方式の両方を統括的に切り替える。
  * **キャンバスクリック/ホバーのモード分岐 (T-0037/T-0039/T-0039b)**: `CanvasDigitizingTool._handle_digitize_click()`/`canvasMoveEvent()`（`map_tool.py`）は`dock_widget.tab2_current_mode`を読み取り分岐する。
    * **新規モード**: 既存点へのスナップ判定(`find_nearest_feature_id`)を一切行わない。ホバー時も判定自体をスキップするため赤枠(`hover_marker`)は常に非表示・カーソルは`Qt.CrossCursor`に固定される(T-0039b)。すべてのクリックはそのまま`canvas_clicked`シグナルで`Tab2DigitizingMixin._on_canvas_clicked()`へ転送され、新規打点処理（後述）が行われる。
    * **編集モード**: 既存点へのスナップ判定のみを行う。ヒット時は`existing_point_selected`シグナルでフォームへ値をバインドし、選択マーカーを表示する（T-0023から変更なし）。ヒットしない（空白）クリックは新規打点を一切行わず、代わりに`blank_click_in_edit_mode`シグナルを発火する。`Tab2DigitizingMixin._on_blank_click_in_edit_mode()`はこれを受けて`_reset_point_selection()`により選択中フィーチャの解除のみを行い、`tab2_current_mode`自体は"edit"のまま変更しない（T-0039。T-0027時点の「空白クリックで選択解除」は、新規モードでは廃止され編集モード専用の挙動として再定義された）。
    * モードトグル自体の切替(`Tab2DigitizingMixin._on_tab2_mode_changed()`)は、編集モードで選択状態(`selected_edit_point_id`)を保持したまま新規モードへ戻る際、`_reset_point_selection()`を呼んで必ず選択状態を破棄する（T-0042。放置すると次の重複チェックの`exclude_feature_id`に古い選択IDが渡り続け、直前に編集した点と同名の点が誤って重複作成可能になるバグがあったため）。
  * **既存点フィールドのリアルタイムコミット (T-0038)**: 編集モードで既存点を選択中、点名/枝番(`edit_point_name`/`edit_point_name_sp`/`edit_branch_no`の`editingFinished`)および出土形態/遺構名/色/属性記号(各コンボの`currentIndexChanged`等)の変更は、確定ボタンを介さずフォーカスアウト・選択変更の都度`_commit_point_identity_if_editing()`/`_commit_attribute_fields_if_editing()`→共通コア`_commit_fields_to_feature()`（`startEditing()`→`changeAttributeValue()`→`commitChanges()`）により即座にGeoPackageへ書き込まれる。新規モード中(`selected_edit_point_id is None`)、既存点フォームへの値読み込み中(`self._suppress_realtime_commit`、`_on_existing_point_selected()`が選択直後の一時的なプログラム的入力の間のみtrueにする)、または①点情報パネルのライブバリデーションがエラー(遺構名未指定/点名重複)を示している間はコミットをスキップする。
...
* **UX/UI フロー**:
  ...
  2. 「対象図面」セレクタで作業対象の画像をアクティブにする。メインキャンバス上で打刻時、対象画像上のローカル座標(`pixel_x`, `pixel_y`)が取得され保存される。
     ...
     * **自動連番/解除トグル（T-0036/T-0040）**: 新規モードのボタンエリアにある[自動連番][解除]の疑似トグル(`tab2_autonum_mode`)が「解除」の場合、上記の自動採番（SPの空欄提示を含む）は行われず、①パネルの点名欄は直前の値のまま据え置かれる。この状態でキャンバスをクリックすると、①パネルの値を直接使わず、クリック地点付近にポップアップする`PointNameEntryDialog`（T-0040、詳細は後述「新規打刻とデータ永続化」）で点名/枝番を入力する。
  3. **編集モード**でのみ、既存の点にマウスを近づけると赤い枠（`CanvasDigitizingTool.hover_marker`）で強調され、クリックすると属性が①点情報パネルへ読み込み表示される（**T-0037/T-0039b**: 新規モードではスナップ判定自体を行わないため、ホバー強調・クリックによる選択のいずれも発生しない）。**T-0023**: クリックして選択した点は、選択専用マーカー（`CanvasDigitizingTool.selected_marker`、`hover_marker`と同じ赤枠スタイル）によりホバーが外れても選択解除まで常時強調表示され続ける。選択の解除は、編集モードのまま「別の点を選択」するか「地図上の空白（フィーチャの無い場所）をクリック」することで行う（新規フィーチャは作成されない。**T-0039**: 空白クリックは`blank_click_in_edit_mode`シグナル経由で`_on_blank_click_in_edit_mode()`→`_reset_point_selection()`を呼ぶのみで、モードは編集のまま維持される）。新規モードへ切り替えた場合も、選択中の点があれば自動的に選択解除される（**T-0042**、後述）。
     * **T-0023/T-0032/T-0038: 既存点の削除・項目編集**: 既存点選択中は`combo_drawing_name`（対象図面）のみが自動的に無効化され（`Tab2DigitizingMixin._set_category_widgets_locked()`、`_CATEGORY_LOCK_WIDGET_NAMES`）、出土形態・遺構名・属性・カラー・点番号（本体番号・枝番）は編集モードのボタンエリアに切り替わった①点情報パネル上でそのまま編集可能である。値の変更はT-0038によりフォーカスアウト・選択変更のたびにリアルタイムでフィーチャへコミットされ（前述「T-0036〜T-0042」段落）、旧来のような確定ボタンや確認モーダルは介さない。「削除」ボタンは確認ダイアログなしで即座に`deleteFeature()`する（旧確認ダイアログは廃止）。属性・出土形態・遺構名の再割当てはリアルタイムコミットで対応済み（旧「削除して打刻し直す運用」の制約は解消された）。
...
* **主要ロジック & State**:
  ...
  * **ホバー判定と既存点選択 (CanvasDigitizingTool)**:
    ...
    * **T-0037/T-0039b**: この近傍探索自体が、`tab2_current_mode`が編集モードの時のみ実行される（新規モードではホバー・クリックのいずれでもスキップされる。前述「T-0036〜T-0042」段落を参照）。
    * クリック時に `existing_point_selected` シグナルを発火し、ドック側でフォームに値をバインド。
  * **新規打刻とデータ永続化 (Step3以降: main_dock.py側の責務)**:
    * 新規モードでのクリックは、CanvasDigitizingToolが`canvas_clicked`シグナルでキャンバス座標のみを通知する。`Tab2DigitizingMixin._on_canvas_clicked()`はまず`tab2_autonum_mode`（自動連番/解除トグル、T-0036）を確認し、
      * **自動連番**: 入力値の検証、リアルタイム重複チェック（`core_logic.check_point_duplicate`。①パネルの色/文言で事前提示済みのため、確定時はサイレントにブロックするのみでダイアログは出さない、T-0032）を経て、①②パネルの現在値のまま`_create_digitized_point_from_state()`（フィーチャ組み立て`core_logic.build_digitized_feature`・GeoPackage書き込みを行う共通コア。新規モードと後述の解除モードの双方から呼ばれる、T-0040で抽出）を呼ぶ。
      * **解除**: `_apply_next_point_number()`による自動採番・SP欄の空欄クリアが行われていないため（①パネルの点名欄が古い値または空欄の可能性がある）、`_handle_release_mode_click()`が代わりにクリック地点付近へ`PointNameEntryDialog`（`main_dock_dialogs.py`、T-0040）をポップアップし、点名/枝番を専用入力させる。属性がSPの場合は`edit_point_name_sp`と同じ正規表現(`^[A-Za-z0-9_-]+$`)のQLineEdit、それ以外はQSpinBox（初期値は同一出土形態/遺構名グループ内の直近作成点名、`_get_last_created_point_name()`、T-0041）に切り替わる。OKで`core_logic.check_point_duplicate`による重複チェックを行い、重複時はOK/キャンセルボタンの上に赤字（`点名重複エラー: {識別子}`形式）で表示して閉じない。出土形態・遺構名・色・属性は①②パネルの現在値をそのまま使う。
      いずれの経路でも、新規遺構名のクリック時自動登録は行わず、②属性パネルの「作成」ダイアログでの事前作成に一本化されている（T-0027から変更なし）。
