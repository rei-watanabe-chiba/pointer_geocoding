...
| T-0036 | UX改善1/3: tab2先頭に[新規][編集]モード切替トグル(build_segmented_toggle方式)を新設。点情報パネルのボタンエリアをモード連動で切替: 新規=[自動連番][解除]トグル(解除選択時は現状のcore_logic自動採番をスキップし手動入力可能に、SP属性選択時は自動的に解除相当扱い)、編集=[削除]ボタンのみ | 人手確認待ち | `.claude/logs/implement/2026-09-16-T-0036-mode-toggle-and-button-area.md` | 静的検証: 問題なし。実行環境での確認が必要 | - |
| T-0037 | UX改善2/3: T-0036のモードに応じてキャンバスクリック挙動を変更。新規モード=現状通り打点優先。編集モード=打点無効化・既設フィーチャへのスナップ判定のみ有効(クリックで選択のみ)。既存の「キャンバス空白クリックでの選択解除」機能を廃止 | 承認待ち | - | - | - |
