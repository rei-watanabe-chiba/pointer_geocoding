...
| T-0030 | T-0029フォローアップ: CSVファイルが読み込めない場合（パス不正・読込失敗）、「CSVファイル利用」ラジオボタンを無効化。選択中だった場合は「新規作成・更新」へ強制切替。この際、原点/X範囲/Y範囲の入力値は一切変更せず直前の値を維持（値が変わるのはCSV読込が成功した時のみ） | 実装中 | - | - | - |
