...
| T-0033 | T-0032 UI調整: ①点情報/属性/フォーカスモード(マスター制御)のQGroupBoxタイトルを削除しHLine区切りに変更②点情報パネルのステータスをフラットテキスト化③行ごとのウィジェットをやめ複数行QLabelにまとめて表示④点名/枝番インプット・点名変更/削除/属性変更ボタンをパネル外に配置、パネル自体はstart_dialog方式の左ボーダー色分けに変更⑤カラーボタン無効時はグレー表示に変更⑥透明度「更新」ボタンを削除しスライダーのリアルタイム反映のみに戻す⑦点名重複/遺構名未指定エラー時に該当入力欄(点名インプット/遺構名セレクタ)にも赤枠を付与 | 承認待ち | - | - | - |
