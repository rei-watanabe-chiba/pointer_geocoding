...
| T-0026 | 起動時ダイアログ改修: 「グリッド数」設定をX範囲・Y範囲（最小〜最大）に置換。Y軸は半角英大文字1〜2桁対応のカスタムスピンボックス(ExcelColumnSpinBox)。グリッドCSV出力を指定範囲内のみに変更。プレビュー機能は維持。T-0028でAttributeError修正済み、原点座標とXY範囲の独立性も調査で確認済み | 人手確認待ち | `.claude/logs/implement/2026-09-15-T-0026-start-dialog-xy-range.md` | 静的検証: 問題なし。原点/範囲の独立性を追加調査でも確認 | - |
...
| T-0029 | T-0026追加機能: グリッドCSV選択欄の下に「新規作成・更新」／「CSVファイル利用」ラジオボタンを追加。新規作成・更新時は原点/X範囲/Y範囲が常時編集可能でCSVパス変更の都度強制上書き。CSVファイル利用時は原点/範囲を編集不可（有効なCSV未選択時は無効化）。モード切替時、直前の値はクリアせず編集可能な初期値として流用。初期選択は「新規作成・更新」。`grid_config`に`grid_mode`追加、`use_existing_csv`判定の不整合も解消 | 人手確認待ち | `.claude/logs/implement/2026-09-15-T-0029-grid-mode-radio.md` | 静的検証: 問題なし。実行環境での確認が必要 | - |
| T-0030 | T-0029フォローアップ: CSVファイルが読み込めない場合（パス不正・読込失敗）、「CSVファイル利用」ラジオボタンを無効化。選択中だった場合は「新規作成・更新」へ強制切替。この際、原点/X範囲/Y範囲の入力値は一切変更せず直前の値を維持（値が変わるのはCSV読込が成功した時のみ） | 人手確認待ち | `.claude/logs/implement/2026-09-15-T-0030-grid-mode-csv-invalid-guard.md` | 静的検証: 問題なし。実行環境での確認が必要 | - |
| T-0031 | グリッド設定の警告UI統一: ①生成予定/CSV読込行数が10,000件超で緑パネル(panel_preview_status)を警告色に切替、簡潔文言+「確認」ボタン表示、確認押下でボタン消滅・警告解除・セッション開始(btn_ok)を有効化。値変更の都度リセットし1万超のままなら再出現。②グリッド設定関連のQMessageBox（CSV不正・X/Y範囲min>max等）を全て緑パネル方式に統一、確認押下までセッション開始を無効化。CSV不正時は確認押下でCSV選択欄クリア（原点/範囲の値は保持） | 人手確認待ち | `.claude/logs/implement/2026-09-16-T-0031.md` | 静的検証: 問題なし。実行環境での確認が必要 | - |
