...
| T-0039 | UX追加1/2: 編集モードでの空白クリック時、編集モードは維持したまま選択中フィーチャの選択を解除する。map_tool.pyから新シグナルをemitし、tab2_digitizing_mixin.py側で既存の`_reset_point_selection()`を呼ぶ形で実装(新規モードのスナップ無効化は調査の結果、既に正しく実装済みと確認済みのため対応不要) | 実装中 | - | - | - |
