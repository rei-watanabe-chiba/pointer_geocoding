...
| T-0021 | UI改修(T-0020の設計修正): アイコンレール＋サイドパネルを`MainDockWidget`内部レイアウトから分離し、新設の軽量な左ドックウィジェット（`Qt.LeftDockWidgetArea`）へ格納。`MainDockWidget`（右ドック、`Qt.RightDockWidgetArea`）は保存ボタン＋遺物点作成エリアのみを保持。左ドック内のウィジェットは引き続き`MainDockWidget`のメソッド（`self`）に紐づけたまま、QDockWidgetという入れ物だけを差し替える設計とし、T-0020のロジック・状態管理は極力再利用する | 実装中 | - | - | - |
