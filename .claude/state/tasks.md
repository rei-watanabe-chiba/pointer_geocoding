...
| T-0036 | UX改善1/3: tab2先頭に[新規][編集]モード切替トグル(build_segmented_toggle方式)を新設。点情報パネルのボタンエリアをモード連動で切替: 新規=[自動連番][解除]トグル(解除選択時は現状のcore_logic自動採番をスキップし手動入力可能に、SP属性選択時は自動的に解除相当扱い)、編集=[削除]ボタンのみ | 完了 | `.claude/logs/implement/2026-09-16-T-0036-mode-toggle-and-button-area.md` | 静的検証: 問題なし | 完了 |
| T-0037 | UX改善2/3: T-0036のモードに応じてキャンバスクリック挙動を変更。新規モード=スナップ判定を完全スキップし常に新規打点。編集モード=打点無効化・既設フィーチャへのスナップ判定のみ有効(クリックで選択のみ、空白クリックは無反応)。既存の「キャンバス空白クリックでの選択解除」機能を廃止 | 完了 | `.claude/logs/implement/2026-09-16-T-0037-mode-aware-canvas-click.md` | 静的検証: 問題なし | 完了 |
| T-0038 | UX改善3/3: 編集モードでの点名/枝番/属性系(出土形態・遺構名・属性)の変更をフォーカスアウト時コミット方式でリアルタイム反映化。共通ヘルパー`_commit_fields_to_feature()`に集約。既存の「点名変更」「属性変更」確定ボタンを廃止(編集モードのボタンエリアは削除ボタンのみに)。重複判定・出土形態未指定判定は既存のリアルタイム判定ロジックを流用しコミット前ガードとして再利用。カラーは既存の即時反映のまま | 人手確認待ち | `.claude/logs/implement/2026-09-16-T-0038-realtime-field-commit.md` | 静的検証: 問題なし(軽微指摘: _suppress_realtime_commitがtry/finallyでない点、リスク低のため許容)。実行環境での確認が必要 | - |
