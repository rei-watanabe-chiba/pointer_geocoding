...
| T-0041 | UX軽微改修4点: ①②④はユーザー確認済み。③のQSpinBoxへのグローバルQSS追加(border三角形トリック含む)が`docs/integrated_master_design.md`23-24行目の「QSpinBoxはOSネイティブUI保護のためQSS適用対象外」という設計原則に違反しており、起動時ダイアログ等の他のQSpinBoxを破壊する回帰を引き起こした。QSpinBox用QSS一式を全て削除しOSネイティブ描画に戻す差し戻しが必要 | 実装中 | `.claude/logs/implement/2026-09-16-T-0041-minor-ux-fixes.md` | 設計書違反により差し戻し(ユーザー発見・報告) | ①②④確認済み、③は設計書違反のため要修正 |
