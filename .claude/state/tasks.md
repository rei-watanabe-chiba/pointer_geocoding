...
| T-0045-b | tab1におけるCoreUI化・機能分離の追加試行(②座標変換後ダイアログ廃止／③`src/ui/core/validators.py`新設・tab1適用／④LayerManager高レベルAPI追加)。第一弾は静的検証済み。延長としてValidatorのエラー表示ヘルパー化(`show_validation_error`)を実装中。詳細は`.claude/state/v2-coreui-plan.md`「T-0045-b」節 | 実装中 |
