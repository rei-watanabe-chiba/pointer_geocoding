...
| T-0041 | UX軽微改修4点: ①PointNameEntryDialog(解除モード)の点名初期値を直前の点名でプリセット(`_get_last_created_point_name()`新設)②重複エラー文言を「点名重複エラー: [識別子]」形式に統一③QSpinBox矢印つぶれの原因(style_helper.pyのQSSでsubcontrol幅未定義)を特定し共通スタイルに矢印サイズ明示を追加④titlelessプロパティでgroup_point_info(タイトルなしQGroupBox)のmargin-top/padding-topのみ縮小 | 静的検証中 | `.claude/logs/implement/2026-09-16-T-0041-minor-ux-fixes.md` | - | - |
