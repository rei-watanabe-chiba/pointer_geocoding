...
| T-0037 | UX改善2/3: T-0036のモードに応じてキャンバスクリック挙動を変更。新規モード=スナップ判定を完全スキップし常に新規打点。編集モード=打点無効化・既設フィーチャへのスナップ判定のみ有効(クリックで選択のみ、空白クリックは無反応)。既存の「キャンバス空白クリックでの選択解除」機能を廃止 | 人手確認待ち | `.claude/logs/implement/2026-09-16-T-0037-mode-aware-canvas-click.md` | 静的検証: 問題なし。実行環境での確認が必要 | - |
| T-0038 | UX改善3/3: 編集モードでの点名/枝番/属性系(出土形態・遺構名・属性)の変更をフォーカスアウト時コミット方式でリアルタイム反映化。既存の「点名変更」「属性変更」確定ボタンを廃止。重複判定・出土形態未指定判定は既存のリアルタイム判定ロジックを流用。カラーは既存の即時反映のまま。フォーカスモードの透明度表示は現状維持(自動切替なし) | 承認待ち | - | - | - |
