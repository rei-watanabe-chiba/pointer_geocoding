...
| T-0044 | QSpinBoxへの安全なQSS適用パターン検証。ユーザーの人手確認で不具合発見: up-button/down-buttonルールに`subcontrol-position`が未指定のため、ボタンが視覚的にボックス外へ分離し、クリック判定領域もズレてホバー/クリックが効かない不具合。矢印サブコントロール自体には触れていないためT-0041とは別種の不具合。`subcontrol-position: top right`/`bottom right`の追加と、テキスト領域とボタンの重なりを避ける`padding-right`調整で修正中 | 実装中 | `.claude/logs/implement/2026-09-16-T-0044-qspinbox-safe-qss.md` | - | - |
