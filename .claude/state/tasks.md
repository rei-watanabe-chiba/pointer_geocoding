...
| T-0036 | UX改善1/3: tab2先頭に[新規][編集]モード切替トグル(build_segmented_toggle方式)を新設。点情報パネルのボタンエリアをモード連動で切替: 新規=[自動連番][解除]トグル(解除選択時は現状のcore_logic自動採番をスキップし手動入力可能に、SP属性選択時は自動的に解除相当扱い)、編集=[削除]ボタンのみ | 実装中 | - | - | - |
