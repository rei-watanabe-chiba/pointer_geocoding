...
| T-0032 | T-0027フォローアップ大改修: 点情報パネル(出土形態/点名+枝番/XY座標の3行、背景色で状態表示=青:新規/黄:編集/赤:エラー、最上部ステータス文言)・属性パネル(ラベル変更、作成/カラー別行、パネル順序変更)の再設計。既設点編集で出土形態/遺構名/属性/点名/枝番を全て編集可能化し「点名変更」(点名+枝番)/「属性変更」(出土形態+遺構名+属性)ボタンに分離。PointRenameDialog廃止、点名重複・遺構名未指定をリアルタイム判定してパネル赤表示に統一(QMessageBox/ダイアログ内赤字は全廃止)。重複判定から対象図面フィルタを除去(出土形態内のみで判定)。属性確定ボタン廃止、透明度スライダー横に「更新」ボタン新設(2:1比率)。遺構新規作成後の自動カラーピッカー起動を廃止。スコープ外だがmain_dock.pyのupdate_symbology_opacity()も属性表示文言変更に伴い1箇所修正 | 人手確認待ち | `.claude/logs/implement/2026-09-16-T-0032-tab2-panel-redesign-and-inline-edit.md` | 静的検証: 問題なし。実行環境での確認が必要 | - |
| T-0033 | T-0032 UI調整: ①点情報/属性/フォーカスモード(マスター制御)のQGroupBoxタイトルを削除しHLine区切りに変更②点情報パネルのステータスをフラットテキスト化③行ごとのウィジェットをやめ複数行QLabelにまとめて表示④点名/枝番インプット・点名変更/削除/属性変更ボタンをパネル外に配置、パネル自体はstart_dialog方式の左ボーダー色分けに変更⑤カラーボタン無効時はグレー表示に変更⑥透明度「更新」ボタンを削除しスライダーのリアルタイム反映のみに戻す⑦点名重複/遺構名未指定エラー時に該当入力欄(点名インプット/遺構名セレクタ)にも赤枠を付与 | 人手確認待ち | `.claude/logs/implement/2026-09-16-T-0033-tab2-panel-ui-adjustment.md` | 静的検証: 問題なし。実行環境での確認が必要 | - |
