...
| T-0047 | dialogs.py（実測808行）へCoreUI適用（GridInputDialog/FeatureCreateDialog/PointNameEntryDialog）。verifier静的検証: 問題なし。人手確認中に2件の動作修正要望を検出し追加対応: ①属性がSPから別属性へ切替時に自動連番モードへ自動復帰（tab2_plot.py `_update_autonum_toggle_for_sp`）②PointNameEntryDialogの確定時QMessageBoxバリデーションをリアルタイムインライン表示（dialogs.py、UIStyleHelper.create_status_panel/update_status_panel使用）に変更。実装ログ: `.claude/logs/implement/2026-09-17-T-0047-coreui-dialogs.md`、追加修正ログ: `.claude/logs/implement/2026-09-17-T-0047-fix-autonum-and-inline-validation.md`、チェックリスト: `.claude/logs/implement/2026-09-17-T-0047-coreui-dialogs-checklist.md`、`.claude/logs/implement/2026-09-17-T-0047-fix-autonum-and-inline-validation-checklist.md`。さらに追加修正: FeatureCreateDialogの確定時QMessageBoxバリデーションも同様にリアルタイムインライン表示（UIStyleHelper.create_status_panel/update_status_panel）へ変更。実装ログ: `.claude/logs/implement/2026-09-17-T-0047-fix-featurecreatedialog-inline-validation.md`、チェックリスト: `.claude/logs/implement/2026-09-17-T-0047-fix-featurecreatedialog-inline-validation-checklist.md`。verifier静的検証（追加修正2件含む）: 問題なし。実行環境での確認が必要。さらに人手確認で、FeatureCreateDialog/PointNameEntryDialog(SP属性時)ともリアルタイムバリデーションが効かず確定ボタン押下時にしかエラー表示が切り替わらない不具合を再検出。textChanged接続自体の静的なロジックには不備が見つからなかったため、フォールバックとしてtextEdited(ユーザー入力由来のキー入力ごとに確実に発火するQt標準シグナル)をtextChangedと併用接続する対応を実施(dialogs.py)。実装ログ: `.claude/logs/implement/2026-09-17-T-0047-fix-realtime-validation-textedited.md`、チェックリスト: `.claude/logs/implement/2026-09-17-T-0047-fix-realtime-validation-textedited-checklist.md`。デバッグログ(`QgsMessageLog`、コミット`82c48a8`)による実機切り分けの結果、真因はシグナル配線不良ではなく「非SP属性(S/P/C)の重複チェック(`DuplicateValidator`)がそもそもリアルタイム接続されていない実装スコープの見落とし」と判明。`PointNameEntryDialog`に`_check_duplicate`を新設しSP/非SP共通でリアルタイム重複チェックを行うよう修正し、デバッグログも除去。実装ログ: `.claude/logs/implement/2026-09-17-T-0047-fix-nonsp-duplicate-realtime-validation.md`、チェックリスト: `.claude/logs/implement/2026-09-17-T-0047-fix-nonsp-duplicate-realtime-validation-checklist.md`。verifier静的検証（非SP属性リアルタイム重複チェック追加分）: 問題なし。実行環境での確認が必要 | 人手確認待ち |
