...
| T-0044 | QSpinBoxへの安全なQSS適用パターン検証。人手確認3回目でボーダートリック矢印が黒塗りの四角形になる不具合判明。Web調査でQtのborder-triangleハックはQSpinBox矢印に対し既知の不安定挙動と判明(Qt Forum等)。`image`プロパティ+インラインSVG(base64データURI、ニュートラルグレー#6B6B6B固定、テーマ非追従)方式へ転換して修正中 | 実装中 | `.claude/logs/implement/2026-09-16-T-0044-qspinbox-safe-qss.md` | - | - |
