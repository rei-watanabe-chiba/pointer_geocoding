...
| T-0020 | UI改修（ユーザー提案）: 図面管理・設定タブをQTabWidgetから分離し、左端アイコン列＋折りたたみ可能な左パネル（VSCodeアクティビティバー式、固定幅、アイコン再クリックで閉じる）へ再構成。遺物点作成タブはタブなしの常時表示メインエリアに変更。図面管理の情報パネルは左パネル最下部へ移動し遺物点作成の情報パネルと分離。`_on_tab_changed()`相当のロジック（マップツール有効/無効切替等）をパネル開閉状態に応じて再設計。T-0018の人手確認とは独立して進行可 | 実装中 | - | - | - |
