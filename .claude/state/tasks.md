...
| T-0029 | T-0026追加機能: グリッドCSV選択欄の下に「新規作成・更新」／「CSVファイル利用」ラジオボタンを追加。新規作成・更新時は原点/X範囲/Y範囲が常時編集可能でCSVパス変更の都度強制上書き。CSVファイル利用時は原点/範囲を編集不可（有効なCSV未選択時は無効化）。モード切替時、直前の値はクリアせず編集可能な初期値として流用。初期選択は「新規作成・更新」。`grid_config`に`grid_mode`追加、`use_existing_csv`判定の不整合も解消 | 静的検証中 | `.claude/logs/implement/2026-09-15-T-0029-grid-mode-radio.md` | - | - |
