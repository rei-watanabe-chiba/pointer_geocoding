...
| T-0044 | QSpinBoxへの安全なQSS適用パターン検証。人手確認2回目でボタン位置/クリックは正常化したが矢印グリフが非表示と判明(ボタンにカスタムジオメトリを与えた時点で矢印側も明示描画しない限り何も表示されないという新知見。T-0041の消失もボタン位置指定漏れが真因だった可能性)。ユーザー許可のもと`::up-arrow`/`::down-arrow`にボーダートリック(`palette(text)`使用、テーマ非依存)で矢印を明示追加して修正中 | 実装中 | `.claude/logs/implement/2026-09-16-T-0044-qspinbox-safe-qss.md` | - | - |
