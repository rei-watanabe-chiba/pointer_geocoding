...
| T-0020 | UI改修（ユーザー提案）: 図面管理・設定タブをQTabWidgetから分離し、左端アイコン列＋折りたたみ可能な左パネル（VSCodeアクティビティバー式、固定幅、アイコン再クリックで閉じる）へ再構成。遺物点作成タブはタブなしの常時表示メインエリアに変更。図面管理の情報パネルは左パネル最下部へ移動し遺物点作成の情報パネルと分離。`_on_tab_changed()`相当のロジック（マップツール有効/無効切替等）をパネル開閉状態に応じて再設計。設計書も追随更新 | 再オープン(T-0021へ) | `.claude/logs/implement/2026-09-15-T-0020-side-panel-nav.md` | 静的検証問題なし | ユーザー確認の結果、「1つの右ドック内でのレイアウト分割」になっており、要望（QGISドックエリア自体を左右分割、中央にキャンバス）と異なると判明。T-0021で対応 |
| T-0021 | UI改修(T-0020の設計修正): アイコンレール＋サイドパネルを`MainDockWidget`内部レイアウトから分離し、新設の軽量な左ドックウィジェット（`Qt.LeftDockWidgetArea`）へ格納。`MainDockWidget`（右ドック、`Qt.RightDockWidgetArea`）は保存ボタン＋遺物点作成エリアのみを保持。左ドック内のウィジェットは引き続き`MainDockWidget`のメソッド（`self`）に紐づけたまま、QDockWidgetという入れ物だけを差し替える設計とし、T-0020のロジック・状態管理は極力再利用する | 人手確認待ち | `.claude/logs/implement/2026-09-15-T-0021-dock-split.md` | 静的検証問題なし | - |
