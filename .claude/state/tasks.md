...
| T-0038 | UX改善3/3: 編集モードでの点名/枝番/属性系(出土形態・遺構名・属性)の変更をフォーカスアウト時コミット方式でリアルタイム反映化。共通ヘルパー`_commit_fields_to_feature()`に集約。既存の「点名変更」「属性変更」確定ボタンを廃止(編集モードのボタンエリアは削除ボタンのみに)。重複判定・出土形態未指定判定は既存のリアルタイム判定ロジックを流用しコミット前ガードとして再利用。カラーは既存の即時反映のまま | 静的検証中 | `.claude/logs/implement/2026-09-16-T-0038-realtime-field-commit.md` | - | - |
