...
| T-0031 | グリッド設定の警告UI統一: ①生成予定/CSV読込行数が10,000件超で緑パネル(panel_preview_status)を警告色に切替、簡潔文言+「確認」ボタン表示、確認押下でボタン消滅・警告解除・セッション開始(btn_ok)を有効化。値変更の都度リセットし1万超のままなら再出現。②グリッド設定関連のQMessageBox（CSV不正・X/Y範囲min>max等）を全て緑パネル方式に統一、確認押下までセッション開始を無効化。CSV不正時は確認押下でCSV選択欄クリア（原点/範囲の値は保持） | 実装中 | - | - | - |
